...
export async function POST(req: NextRequest) {
  try {
    ...
    const targetUrl = `${TARGET_SERVER_BASE_URL}/api/wiki/page`;

    // Extract repo info and other parameters
    const { repoInfo, page, language, provider, model } = requestBody;
    ...
    console.error('Error in API route (/api/wiki/page):', error);
